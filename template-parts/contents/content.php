...
$show_excerpt = isset($masco['show_excerpt']) ? $masco['show_excerpt'] : false;
...
<div id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	<div class="single-post-item <?php echo esc_attr($grid)  ?>">
			<div class="post-thumbnail-wrapper">
				<?php
				if (is_sticky()) {
					echo '<span class="sticky-text" >' . esc_html__('Sticky', 'masco') . '</span>';
				}
				?>
		...
		<div class="post-content">
			...
			<?php if ($show_excerpt) {
				echo '<p>' . esc_html($mascoObj->postExcerpt($limit, get_the_excerpt())) . '</p>';
			} ?>

			<div class="post-read-more">
				<a href="<?php echo esc_url(get_permalink()); ?>">
					<?php echo (isset($masco['continue_reading_title'])) ? $masco['continue_reading_title'] : esc_html__('Read More', 'masco') ?>
