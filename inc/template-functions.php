 ...
 * @return array
 */
function masco_body_classes($classes)
{
...
add_filter('body_class', 'masco_body_classes');

/**
 * Add a pingback url auto-discovery header for single posts, pages, or attachments.
 */
function masco_pingback_header()
{
    if (is_singular() && pings_open()) {
        printf('<link rel="pingback" href="%s">', esc_url(get_bloginfo('pingback_url')));
    }
}
add_action('wp_head', 'masco_pingback_header');

function masco_dd($var)
{
    echo '<pre>';
    print_r($var);
    echo '</pre>';
 ...
 */
function masco_detect_homepage()
{
...
function masco_get_site_logo()
{
    ...
    } else if (!empty($masco['logo']['url'])) {
        $logo_url = esc_url($masco['logo']['url']);
        $logo = '<img src="' . esc_url($logo_url) . '" alt="' . esc_attr(get_bloginfo('title')) . '" class="navbar-brand__regular">';
    } else {
        if (has_custom_logo()) {
 ...
 * Get the site logo for Bufet
 */
function masco_get_site_sticky_logo()
{
    ...
    } else if (!empty($masco['sticky_logo']['url'])) {
        $logo_url = esc_url($masco['sticky_logo']['url']);
        $logo = '<img src="' . esc_url($logo_url) . '" alt="' . esc_attr(get_bloginfo('title')) . '" class="navbar-brand__sticky">';
    }
    return $logo;
}

add_action('wp_ajax_cloadmore', 'masco_comments_loadmore_handler'); // wp_ajax_{action}
add_action('wp_ajax_nopriv_cloadmore', 'masco_comments_loadmore_handler'); // wp_ajax_nopriv_{action}

function masco_comments_loadmore_handler()
{
...
/**
 * masco Is edit mode in elementor
 *
 */
function masco_is_edit_mode(  ) {

    return isset($_GET['elementor-preview']);
}
/**
 * masco header Settings
 *
 */
function masco_header_settings() {

    if (defined('ELEMENTOR_PRO_VERSION') && masco_is_edit_mode() ) {
        return;
    }else{
        $masco = get_option( 'masco' );

        $check_header_post = get_posts( ['post_type' => 'masco_header'] );

        if ( 0 != count( $check_header_post ) ) {
            printf( '<header class="site-header masco-elementor-header">' );
            masco_header_footer_template_query( 'masco_header' );
            printf( '</header>' );
        } else {
...
/**
 * masco Footer Settings
 *
 */
function masco_footer_settings() {
    if (defined('ELEMENTOR_PRO_VERSION') && masco_is_edit_mode() ) {
        return;
    }else{
        $check_footer_post = get_posts( ['post_type' => 'masco_footer'] );

        if ( 0 != count( $check_footer_post ) ) {

            masco_header_footer_template_query( 'masco_footer' );
        } else {
            masco_raw_footer();
        }
    }
 ...
 *
 */
function masco_raw_footer()
{
    $masco = get_option('masco');

    if (isset($masco['footer_copyright'])) {
        echo '<div class="masco-copyright text-center">' . $masco['footer_copyright'] . '</div>';
    } else {
        echo '<div class="masco-copyright text-center">' . esc_html__('Copyright 2023, All Rights Reserved', 'masco') . '</div>';
    }
}
 ...
 *
 */
function masco_header_footer_template_query($post_type,  $post_id = '')
{
...
/**
 * masco get archive post type
 *
 */
function masco_get_archive_post_type()
{
    $postname = isset(get_queried_object()->name) ? get_queried_object()->name : '';
    return is_archive() ? $postname : '';
}

function masco_update_elementor_scheme($colors = array())
{
    if (class_exists('ReduxFrameworkPlugin')) :
        $accent_color = $masco['custom_accent_color'];
        $heading_color = $masco['heading_color'];
        $text_color = $masco['text_color'];
        $colors = [
            "1" => "$heading_color",
            "2" => "$heading_color",
            "3" => "$text_color",
...
add_action('after_switch_theme', 'masco_update_elementor_scheme');
...
function masco_preloader()
{
    ...
    if (isset($masco['enable_preloader'])) {
        if (true == $masco['enable_preloader']) {
            printf('%s', $preloader);
        }
    } else {
        printf('%s', $preloader);
