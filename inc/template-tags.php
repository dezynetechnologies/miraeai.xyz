...
if (!function_exists('masco_posted_on')) :
	/**
	 * Prints HTML with meta information for the current post-date/time.
	 */
	function masco_posted_on()
	{
...
if (!function_exists('masco_posted_by')) :
	/**
	 * Prints HTML with meta information for the current author.
	 */
	function masco_posted_by()
	{
		$byline = sprintf(
			/* translators: %s: post author. */
			esc_html_x('%s', 'post author', 'masco'),
...
if (!function_exists('masco_entry_footer')) :
	/**
	 * Prints HTML with meta information for the categories, tags and comments.
	 */
	function masco_entry_footer()
	{
		// Hide category and tag text for pages.
		if ('post' === get_post_type()) {
			/* translators: used between list items, there is a space after the comma */
			$categories_list = get_the_category_list(esc_html__(' ', 'masco'));
			if ($categories_list) {
				/* translators: 1: list of categories. */
				printf('<span class="cat-links"><strong>' . esc_html__('Posted in:', 'masco') . '</strong> ' . esc_html__('%1$s', 'masco') . '</span>', $categories_list); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped

			}
			?>
			<div class="post-bottom d-flex align-items-center justify-content-between">
				<div class="masco-post-author">
					<p class="mb-0">By: <span><?php echo get_the_author_meta('display_name', $author_id); ?></span></p>
				</div>
					<div class="masco-post-comments d-flex align-items-center justify-content-between">
			...
			<?php
			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list('', esc_html_x('', 'list item separator', 'masco'));
			if ($tags_list) {
				/* translators: 1: list of tags. */
				printf('<span class="tags-links"><strong>' . esc_html__('Tagged:', 'masco') . '</strong>' . esc_html__('%1$s', 'masco') . '</span>', $tags_list); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
		}
			...
			comments_popup_link(
				sprintf(
					wp_kses(
						...
						__('Leave a Comment<span class="screen-reader-text"> on %s</span>', 'masco'),
						array(
							'span' => array(
								'class' => array(),
							),
		...
		edit_post_link(
			sprintf(
				wp_kses(
					...
					__('Edit <span class="screen-reader-text">%s</span>', 'masco'),
					array(
						'span' => array(
							'class' => array(),
						),
...
if (!function_exists('masco_post_thumbnail')) :
	/**
...
if (!function_exists('masco_cpt_categories')) :
	/**
	 * This for showing custom post type categories
	 */
	function masco_cpt_categories($taxonomy)
	{
