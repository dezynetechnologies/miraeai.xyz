...
namespace MascoTheme\Inc\Classes;
...
class Masco_Main
{
    ...
    public function pagination()
    {
        ...
        echo '<div class="col-12"><div class="masco-navigation">';
    ...
    public function masco_theContentWithComment()
    {
    ...
    public function masco_breadcrumb_bridge()
    {
        $masco = get_option('masco');

        if (isset($masco['breadcrumb_on'])) :
            if ($masco['breadcrumb_on'] == true) :
                $this->masco_get_the_breadcrumbs();
            endif;
        else :
            $this->masco_get_the_breadcrumbs();
        endif;
     ...
     * @return breadcrumb
     */
    public function masco_get_the_breadcrumbs()
    {
        $masco = get_option('masco');
        $title = $this->generateBreadCrumbTitle();


        if(!empty( $masco['blog_breadcrumbs_img']['url'] ) ){
            $masco_blog_image = $masco['blog_breadcrumbs_img']['url'];
       }else{
            $masco_blog_image = ''; 
       }
        
       $output = '<div class="blog-breadcrumb" style="background-image: url('. $masco_blog_image .')">
    ...
    public function generateBreadCrumbTitle()
    {
        ...
        if (is_home() || is_front_page()) {
            $title = isset($masco['bp_title']) ? esc_html($masco['bp_title']) : esc_html__('Our Blog', 'masco');
        } elseif ('case-study' == masco_get_archive_post_type()) {
            $title = isset($masco['cs_title']) ? esc_html($masco['cs_title']) : esc_html__('Case Study', 'masco');
        } elseif ('job' == masco_get_archive_post_type()) {
            $title = isset($masco['job_title']) ? esc_html($masco['job_title']) : esc_html__(' Current job openings', 'masco');
        } elseif (is_page()) {
            $title = wp_trim_words(get_the_title(), 6, '');
        } elseif (is_single()) {
            $title = $title_count;
        }elseif (function_exists('is_shop') && is_shop()) {
            $title = isset($masco['shop_title']) ? esc_html($masco['shop_title']) : esc_html__('Our Products', 'masco');
        } elseif (is_archive()) {
            $title = get_the_archive_title();
        } elseif (is_search()) {
            $title = esc_html__('Search results for: ', 'masco') . get_search_query();
        }elseif(is_404(  )){
            $title = isset($masco['404_title']) ? esc_html($masco['shop_title']) : esc_html__('404', 'masco');
        }

        return $title;
    }

    public static function masco_breadcrumbs()
    {

        $masco = get_option('masco');

        $sepOpt = (isset($masco['breadcrumb_sep']) ? $masco['breadcrumb_sep'] : '<i class="fa fa-angle-right"></i>');

        /* === OPTIONS === */
        $text['home'] = esc_html__('Home', 'masco'); // text for the 'Home' link
        $text['shop'] = esc_html__('Shop', 'masco'); // text for the 'Shop' link
        $text['category'] = esc_html__('Archive by Category "%s"', 'masco'); // text for a category page
        $text['search'] = esc_html__('Search Results for "%s" Query', 'masco'); // text for a search results page
        $text['tag'] = esc_html__('Posts Tagged "%s"', 'masco'); // text for a tag page
        $text['author'] = esc_html__('Articles Posted by %s', 'masco'); // text for an author page
        $text['404'] = esc_html__('Error 404', 'masco'); // text for the 404 page
        $text['page'] = esc_html__('Page %s', 'masco'); // text 'Page N'
        $text['cpage'] = esc_html__('Comment Page %s', 'masco'); // text 'Comment Page N'
